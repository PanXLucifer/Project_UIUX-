...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Update header auth links when user is logged in
  (function updateAuthHeader() {
    try {
      const currentUser = JSON.parse(localStorage.getItem('archihome_auth') || 'null');
      if (!currentUser) return;

      // Desktop: replace Login text with user's name, change Register into Logout
      const desktopLogin = document.querySelector('a[href="auth/login.html"]');
      const desktopRegister = document.querySelector('a[href="auth/register.html"]');

      if (desktopLogin) {
        desktopLogin.textContent = currentUser.fullname || currentUser.email || 'User';
        desktopLogin.href = '#';
        desktopLogin.id = 'userDisplay';
      }

      if (desktopRegister) {
        desktopRegister.textContent = 'Logout';
        desktopRegister.href = '#';
        desktopRegister.id = 'logoutBtnHeader';
        desktopRegister.addEventListener('click', function (e) {
          e.preventDefault();
          if (confirm('Apakah Anda yakin ingin logout?')) {
            localStorage.removeItem('archihome_auth');
      ...
      const mobileLogin = document.querySelector('#mobileMenu a[href="auth/login.html"]');
      const mobileRegister = document.querySelector('#mobileMenu a[href="auth/register.html"]');

      if (mobileLogin) {
        mobileLogin.textContent = currentUser.fullname || currentUser.email || 'User';
        mobileLogin.href = '#';
        mobileLogin.id = 'mobileUserDisplay';
      }

      if (mobileRegister) {
        mobileRegister.textContent = 'Logout';
        mobileRegister.href = '#';
        mobileRegister.id = 'mobileLogoutBtn';
